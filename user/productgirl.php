...
if (isset($_SESSION['user'])) {
    ...
    $query = "SELECT * FROM sanpham WHERE type = 'quannu' or type = 'aonu'";
...
$query = "SELECT * FROM sanpham WHERE type = 'quannu' OR type = 'aonu' LIMIT $limit OFFSET $offset";
$result = mysqli_query($conn, $query);

// Tính tổng số sản phẩm theo loại
$total_query = "SELECT COUNT(*) AS total FROM sanpham WHERE type = 'quannu' OR type = 'aonu'";
...
<script>
    ...
    if (!checklogin) {  
        event.preventDefault(); // Ngăn chặn form gửi dữ liệu trước khi chuyển hướng
        alert("Bạn phải đăng nhập mới có thể thêm vào giỏ hàng.");
        window.location.href = "login.php"; // Chuyển đến trang đăng nhập
    }
    // Nếu đã đăng nhập, form sẽ được gửi bình thường
}
